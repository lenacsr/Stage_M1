...
ui <-
  ...
  tabsetPanel(
    ...
    tabPanel("Accueil",
            ...
            p("A partir du", strong("transcriptome"), "(l'ensemble des ARN produit) d'une cellule, on peut aussi retrouver des ",strong("voies de signalisations"),", qui désigne une suite de réactions chimiques où un groupe de molécule d'une cellule travaille ensemble pour contrôler des fonctions d'une cellule comme l'apoptose (mort programmé de la cellule) ou la division de la cellule par exemple. Deux voies de signalisations qui vont particulièrement nous intéresser sont", strong("XBP1s")," et ",strong("RIDD"),". Ces voies de signalisation sont activé en cas de stress du",strong ("réticulum endoplasmique")," (une organite de la cellule). Le réticulum endoplasmique peut être stressé en cas de cancer, ou d'une carence en nutriment, ou bien même un choc thermique dans ce cas il active ",strong("IRE1"), "qui est la protéine transmembranaire qui lance les voies de signalisation. Ainsi, lorsque le réticulum endoplasmique est stressé elle active la voie de signalisation XBP1s qui permet d'augmenter la production de protéine utile dans cette période de stress. Elle peut aussi activer la voie de signalisation RIDD qui pour 'soulager' la cellule stressée va détruire les ARNs afin de réduire la production de protéine et de lui donner moins de travail en période de stress."),
  ...
  tabPanel("Déconvolution ICA",
           ...
           p("Cependant, cette méthode n'est pas déterministe donc les résultats à la sortie sont différents. Pour minimiser ce problème, on va chercher à maximiser la stabilité des résultats en utilisant donc un autre algorithme qui trouve la stabilité pour un certains nombre de composantes (qui contient des profiles cellulaire) que l'on choisit à la sortie de notre algorithme d'ICA."),
  ...
  tabPanel("XBP1 et RIDD",
           ...
           p("On va interpréter les résultats de notre déconvolution avec une analyse d'enrichissement.Elle va venir nous permettre d'associer une composante issues de l'ICA précédemment réalisés à une voie de signalisation / état cellulaire. L'analyse d'enrichissement ou 'gene set enrichment analysis' (GSEA) est un outil pour résumer l'information de tableaux d'expression de gènes. Le GSEA nous permet d'avoir un tableau du type."),
           ...
           p("",strong("NES")," = Normalized Enrichment Score, qui indique la force de l'enrichissement. Normalement, un NES positif indique que la signature est enrichie dans la composante ICA, tandis qu'un NES négatif indique que la signature est appauvrie. On a normalisé parce que la pvaleur est ajustée et pour tenir compte de la taille de la signature et du nombre total de gènes."),
           
           p("p-value ajustée (",strong("padj"),") = pvaleur ajustée pour tenir compte des tests multiples."),
           
           titlePanel("Table de résultats du GSEA pour XBP1s19 et RIDD 19"),
           ...
           h4("kmeans"),
           p("Avant d'appliquer les kmeans on va chercher le nombre de groupe idéale qu'on doit utiliser (le k), pour cela on utilise la représentation graphique de l'inertie intra en fonction du nombre de groupe. On utilise la méthode du coude c'est à dire qu'on prends le premier point à partir duquel la pente descende fortement et son abscisse est notre k optimal."),
           
           titlePanel("Représentation graphique de l'inertie intra en fonction du nombre de groupe"),
           
           plotOutput("k_ire1")%>%
             withSpinner(type =6, color = "#7F00FF"),
           
           p("On choisit 2 groupes grâce à la méthode du coude. Pour confirmer cela on a calculé l'indice silhouette qui nous confirme que nous de vons choisir 2 groupes "),
           
           titlePanel("Représentation graphique des groupes trouvés"),
           
           plotOutput("kmeans_ire1")%>%
             withSpinner(type =6, color = "#7F00FF"),
           
           p("On ne voit pas de groupes se former particulièrement"),
           ...
           p("On ne voit pas de groupes se former particulièrement non plus"),
  ...
  tabPanel("CancerSEA",
           ...
           p("Ici nous nous interessons aux signatures des voies de signalisation qu'on retrouve dans Cancer SEA sont : Angiogenèse, Apoptose, Cycle cellulaire, Differentiation, Dégradation de l'ADN, Réparation de l'ADN, l'EMT, l'Hypoxie, l'Inflamation, l'Invasion, la Metastase, la Proliferation, Quiescence, Stemness (La souchitude d'une cellule)"),
           ...
           pickerInput("selected_basenames", "Choisir les étiquettes :",
...
On va tout d'abord procéder à une ACP pour voir le lien entre les différentes variables. Puis on va faire du clustering avec la CAH et les kmeans afin de voir si l'on ne retrouve pas des individus qui ont tendance à se regrouper."),
           ...
           p("On choisit 2 groupes la méthode du coude. Pour confirmer cela on a calculé l'indice silhouette qui nous confirme que nous de vons choisir 2 groupes "),
           ...
           p("",strong("Interprétation"),"On ne voit pas de groupes se former particulièrement"),
           ...
           p("",strong("Interprétation"),"On ne voit pas de groupes se former particulièrement"),
           p("",strong("En conclusion : "),"On ne voit pas de groupe se former à l'issue de notre déconvolution.")
...
On va tout d'abord procéder à une ACP pour voir le lien entre les différentes variables. Puis on va faire du clustering avec la CAH et les kmeans afin de voir si l'on ne retrouve pas des individus qui ont tendance à se regrouper."),
           ...
           h4("kmeans"),
            ...
            p("Avant d'appliquer les kmeans on va chercher le nombre de groupe idéale qu'on doit utiliser (le k), pour cela on utilise la représentation graphique de l'inertie intra en fonction du nombre de groupe. On utilise la méthode du coude c'est à dire qu'on prends le premier point à partir duquel la pente descende fortement et son abscisse est notre k optimal."),
            titlePanel("Représentation graphique de l'inertie intra en fonction du nombre de groupe"),
           plotOutput("k_Hallmarks")%>%
             withSpinner(type =6, color = "#7F00FF"),
           
           p("",strong("Inteprétation :"),"On choisit 2 groupes la méthode du coude. Pour confirmer cela on a calculé l'indice silhouette qui nous confirme que nous de vons choisir 2 groupes "),
  ...
  tabPanel("Qualité signature XBP1s et RIDD",
           ...
           titlePanel("Tableau de contingence des occurences de chaque signatures"),
           uiOutput("contingence_occurence")%>%
             withSpinner(type =6, color = "#7F00FF"),
           
           titlePanel("Tableau de contingence des score cumulés de chaque signatures"),
           uiOutput("contingency_scores")%>%
             withSpinner(type =6, color = "#7F00FF"),
           h3("Distribution des scores lorsqu'IRE1 est actif"),
           p("Ici, nous allons simplement utiliser les scores des signatures XBP1s up et RIDD down, car ce sont des signes que IRE1 est activé. Or, nos échantillons sont des patients atteints du Cancer donc on s'attendrait à ce qu'IRE1 soit activé.
...
Nous allons donc simplement faire la somme des scores de XBP1 UP et RIDD DOWN pour chaque échantillon."),
           ...
           p("Ici on va chercher à savoir si les scores qu'on a attribué aux échantillons respectent une cohérence biologique. En effet on s'attend qu'un patient qui est beaucoup de cellules dans sa tumeur avec XBP1s UP-régulé alors on s'attend à ce qu'il est peu de cellule DOWN régulé. Ainsi quand un échantillon a un score XBP1s UP élevé il est censé avoir un score XBP1s DOWN faible et vice versa. Ainsi nous avons élaboré un indice qui va regarder si les rangs de XBP1s UP et DOWN sont inversés. "),
           titlePanel("indice d'inversion de rang (basé sur la corrélation)"),
           
           layout_column_wrap(
             width = 1,
             value_box(title = "indice inversion de rang entre XBP1S 19 DOWN et UP",
                       showcase = calculator_fill,
                       value = uiOutput("my_box1"),
                       theme = "black" )%>%
               withSpinner(type =6, color = "#7F00FF"),
             
              value_box(title = "indice inversion de rang entre XBP1S pur DOWN et UP",
                     showcase = calculator_fill,
                     value = uiOutput("my_box2"),
                     theme = "black" )%>%
               withSpinner(type =6, color = "#7F00FF"),),
          ...
          p("Lorsqu'IRE1 est actif XBP1s est UP et RIDD est DOWN on va étudier les échantillons où l'on retrouve cela."),
